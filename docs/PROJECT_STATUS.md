...
仓库已在本地冻结 `v0.1.0` 技术预览（三个 npm 包版本 `0.1.0`，tag `v0.1.0`），但
**尚未推送到任何远程仓库**——因此目前仍**没有**第三方可安装或受支持的发布版本。
三个 npm 包保持私有包，不作为 `npm publish`/`npx` 安装通道分发。

发布门进展：第三方接入文档（`quickstart-external.md`）、许可披露
（`THIRD_PARTY_NOTICES.md`）、安全披露（`SECURITY.md`）已就位；完整 bundle 与两层
信任链随交付通道重定移交 `1.0.0` 候选。当前交付通道为 **git clone + `scripts/install-uiv.sh`
安装 CLI**，harness 两制品经 CI 发布至远程 Maven 仓（见 `release-automation.md`）。
在仓库公开并完成首次发布前，仓库中的源码、构建产物和示例工程仍仅供开发验证。
...
- P0-8 Android harness 产品化：工程主体**已完成**（交付制品收敛为 view-dump／
  semantics-dump 两制品，经 `.github/workflows/publish-harness.yml` 由 CI 构建并
  发布至远程 Maven 仓——本机 `publishToMavenLocal` 仅供开发自验，不再是交付通道；
  消费侧接入零构建文件改动——转发由 uiv CLI
  的 init script 自动注入；已发布的 uiv-gradle-plugin:0.1.0-alpha.4 坐标留存不变但
  标记 deprecated，不再随交付演进；仓外 fixture 仅经已发布制品接入、冷道离线跑通；
  golden 四类硬门 View／Semantics／文本布局／bounds；双 lane 信任模型）。原
  **clean online bootstrap gate 已于 2026-07-16 关闭**——等价证据为另一台干净设备
  经远程 Maven 与安装脚本完成真实接入（维护者留档，非本仓自动化复核）；
  `--sandbox --offline` 复用 smoke 亦已于 2026-07-20 补跑留证。
- 待完成：P0-6 全面脱敏（B 轨）、P0-7 B 轨导出树、已登记安全债（`~/.m2` 放行收紧）。
  P0-5 完整 bundle 与两层信任链随交付通道重定移交 `1.0.0` 候选，不再是本预览的阻塞项。
...
的支持矩阵**——发布 workflow 虽已就位，但仓库尚未推送、CI 从未实际运行过，也未做
多机/多平台交叉验证：
...
- [仓外快速接入](quickstart-external.md)：第三方从零到首个 `uiv check` pass 的接入路径。
- [接入指南](onboarding-guide.md)：现有功能的使用与判定语义。
- [发布自动化](release-automation.md)：tag 约定、CI 发布流程与消费者仓库地址形态。
- [发布说明 0.1.0](RELEASE-NOTES-0.1.0.md) 与 [摘要清单](RELEASE-0.1.0-digests.md)：本次技术预览的边界、免责与制品摘要。
- [安全披露](../SECURITY.md)、[第三方许可](../THIRD_PARTY_NOTICES.md)。
