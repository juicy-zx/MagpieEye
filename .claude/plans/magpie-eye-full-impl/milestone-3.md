...
1. **page-report v1 必含 `sessionId: string`**(T3.1b schema 校验三键之一):T3.3 实现 verify-page 时必须接受 `--session <id>` 参数并写入 report 顶层 sessionId(loop 外 standalone 跑允许 'standalone' 字面量);T3.1b 的 runSafeCommand 调用串已含 --session。
2. **re-persist 触发标记消费**(T3.2 定义写 `.magpie/uiv-repersist.json`):消费点归 **T3.1b** 的 execute.ts 接线步骤——在 validateRequirementContract 之前检查该标记,存在则先调 persistRequirementContract(workspaceRoot) 再删标记(幂等);T3.1a 只保证 persist 可带 workspaceRoot 重建。T3.1b 执行 agent 据此在其 S2 步骤内补一小步(含单测:标记在场→合同含 uiParity→标记被清)。
3. **hash 双轨**:scope.sourceDocumentHash 用 magpie 现行 hashContent(sha1,对齐合同 source.hash 漂移告警);mapping.json.sig sidecar 完整性摘要用 sha256(T3.2 写、T3.1b 验)。两者用途不同,不得混用。
4. T3.4 自留的 6 条口径裁量(invariant 是否入 score 分母等)与 T3.3 的"非 base 格暂 render-only"边界:执行前由编排者转 Codex 随子计划审查一并裁定。
