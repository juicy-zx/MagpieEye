...
function validPass(): ReportV1 {
  return {
    ...
    pixel: null,
    structural: { matched: 5, untaggedCoverage: 1, matchRate: 1, missing: [], extra: [], violations: [] },
...
describe('report.json v1 校验器(扩展 v0)', () => {
  ...
  it('合法 inconclusive 件(structural + subReason)原样返回', () => {
    const r: ReportV1 = {
      ...
      structural: { matched: 1, untaggedCoverage: 0.25, matchRate: 0.25, missing: [], extra: [], violations: [] },
      score: 0.5,
    };
    expect(validateReportV1(r)).toEqual(r);
  });
