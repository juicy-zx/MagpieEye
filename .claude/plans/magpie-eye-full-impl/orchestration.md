...
- **编排者模式(用户指令,节省主上下文)**:主会话是编排者,只做四件事——任务分解与派发、验收把关(跑验收命令/抽查产物)、Codex 协同节点、meta.json 状态机维护。一切执行性工作(写代码、跑构建、执行子计划步骤、Figma/其他 MCP 调用——子 agent 经 ToolSearch 自行加载)一律派发子 agent 完成;主会话不亲自实现。并发派发时按路径隔离(各 agent 只 add/commit 自己的路径,meta.json 只由主会话写)。
- **派发模板固定条款(三次实证的坑,每个派发词必带)**:① 禁止"设监视器/后台任务后停机等通知"——已停机的 agent 收不到任何唤醒,必须前台跑长命令(timeout 600000)或轮询后台输出文件保持活跃直至交付;② 精确路径 git add,严禁 add -A(共享工作树);③ 不碰 meta.json/barrel/.gitignore(登记制);④ 30 分钟无法自解即停止报告;⑤ 写偏类验收(改 CalibCard 等共享 fixture)必须与其他写偏任务排他串行。
- **子 agent 模型分层(按任务难度选档,省 token)**:
  - `haiku` —— 纯机械零判断:grep/符号核验(如 T3.0)、文件存在性巡检、产物收集汇总;
  - `sonnet` —— 照单执行:按已批准 bite-size 子计划逐步实现(代码/命令已写死,无需架构判断)、按精确指令清单修订文档、fixture 编写、T1.4 循环中扮演"修正者"的编码 agent(更贴近真实使用场景);
  - `opus` —— 复杂实现与中等疑难:规格已钉死但实现精度/工程复杂度高的任务——L2 引擎与指标公式实现(T1.3)、render-daemon/Gradle Tooling API/UDS(T2.1)、构建疑难首轮调试、含实测解读的任务(NATIVE 钉版本);
  - 继承主模型(fable)—— 判断/设计密集:子计划起草、对抗验证/多视角审查、Codex 审查意见落实中的口径裁量、opus 两轮未解的疑难升级;
  - effort 同步分层:机械任务 low,标准执行 medium(默认),验证/评审/疑难 high。
  - 拿不准时就近升档,验收把关兜底(验收命令不过 = 重派更高档);升档链 haiku→sonnet→opus→fable。
...
- **D-05(2026-07-09)快车道纳入裁定**:Paparazzi fast lane 纳入,定位钉死"静态 @Preview 组件级加速插槽"——不替代慢车道、不进正确性关键路径,任何不可判/语义不可达/配置漂移/崩溃自动回落慢车道;报告必须标注 fast/slow lane 来源。接线任务 T2.8(M2 尾,条件依赖 T2.1:daemon 托管 worker,纯 stdin/stdout 零监听面;T2.1 延期则推 M4)。反射 internal API 钉版本硬门禁:Paparazzi/AGP/Kotlin/LayoutLib 任一升级必重跑 spike 判据(G1/G2/语义可达/对齐/30 轮稳定),未重跑禁升级,重跑失败 fast lane 降 disabled(同 NATIVE hard-gate 模式)。
- **D-06(2026-07-09)熔断语义细化**:熔断(mr<0.8)只抑制降级配对(joinSource=text|lcs)的属性断言;tag 确定性配对永远执行断言;missing 不受熔断门控;v1 校验器约束改为 matching_rate_low ⇒ matchFailure 非空(删除 violations 必空);两个回归测试(4叶缺1 tag 对违规仍出 / 降级主导场景仍抑制)。**并发经验**:T2.1 验收与 T2.7 harness 争用 CalibCard.kt——写偏类验收(equivalence/harness)之间必须排他串行,守卫脚本对脏卡 exit 3 拒绝是正确防线。
- **D-07(2026-07-09)慢车道目标修正(a')**:3.1s 目标废止(建立在"daemon 能摊薄渲染/JVM 成本"的已证伪假设上,系编排计划自设过紧,非架构问题)。新口径:测量协议统一 --rerun(禁 EOF-comment-toggle 类会被 build cache 吃掉的假热路径);热路径 P50 ≤6.0s 且记录 coldP50/hotP50/savedMs/speedupRatio;等价/0600/cwd 拒绝/fallback/lane 正交保持。**M2 北极星修正:慢车道=正确性兜底+常驻编排点;秒级内循环=T2.8 fastlane(实测 15ms)**。T2.1 实测 --rerun 热 5838ms ≤6.0s 达标(冷 10442ms,saved≈4.6s)。发现 2(odiff 悬挂)归 T2.6,验收四条:check 后 stopOdiffServer/异常可靠 fallback spawn/L1 advisory 失败不污染 L2 verdict(exit code 以 L2 为准)/check 完成后进程短超时内退出机判。
- **D-08(2026-07-09)P50 真实口径揭出慢车道退化,立项 T2.9 诊断**:--rerun 真实测量 P50=10140ms(裸 gradle 7656ms vs T2.1 期 5838ms)——M2 后段引入退化,直接调阈值=把未知退化合法化,带病关闭=兜底车道失守。T2.9 口径:两段测(裸 Gradle 定位工程本体退化 / UIV_FASTLANE=0 UIV_RERUN=1 CLI 定位 daemon+CLI ~2.5s 开销);bisect T2.6~T2.8 每候选 commit ≥3 轮取中位,输出 machine-checkable 表(commit/变更点/裸P50/CLI P50/delta);可修复退化(如 fastlane-worker 误入 demo test classpath)先修复再复测;必要成本才带证据修订目标,此前不接受 11s。latency pass:false 标注"诊断前事实",不作 M2 通过证据。M2 审查保持 open 至 T2.9 结论。
- **T2.9 证伪并更正(2026-07-09,Codex 确认追加订正)**:D-08 条目所载"7656 vs 5838 裸口径退化/classpath 增长致 Robolectric 变慢"的推测已被 T2.9 bisect+对照实验**证伪**——5838ms 系旧 EOF-toggle 假热协议误标注为 T2.1 期参考值,裸路径实际零退化(before 5304 vs HEAD 5184);P50=10140 的真因是 odiff-bin 上游 5s 看门狗定时器吊住事件循环,已在 CLI 收尾侧修复(flushAndExit),复测 P50=5090ms ≤6000 达标。commit 879019c message 中的错误推测以本条为准更正。M2 审查通过(round 2),目标终值保持 6000ms。
- **D-09(2026-07-09)M3 接入方案重写(T3.0 重核验驱动,设计文档 5.1 双保险化)**:① 主通道=requirement-type adapter(uiParity 缺失/report 不存在/解析失败/pass≠true/stale 均产 typeFinding,经 'implementation' scope 阻断 completed;adapter 必须读 report 验 schema/pass/关联/新鲜度,不信 path 字符串);辅通道=专属 requiredEvidence 判定单元(计算正确性+报告可见性+防 ANY-of 回归,不再是唯一硬门禁)——根因:decideRequirementCompletion 漂移后 'implementation' scope 豁免 missingEvidence,C9 计算层正确但生效层断链。② verify-page 触发点=implementation 阶段完成路径、validateRequirementContract 之前(原"integration_test 后期"自基线即与 DEFAULT_STAGES 时序矛盾,废弃);每次新改动重跑不复用旧 report,runSafeCommand 失败落缺失态由 adapter 阻断。③ .ui-verify 窄豁免:reports/renders/state 可豁免;mapping.json 受控产物豁免(带写入来源/hash 约束);baselines/spec 不豁免(防模型手改骗验证);验收两面=工具写不被 scope restore 回滚+模型手改不能绕过。④ coverageUnit 复用 'validation_requirement'。⑤ persistRequirementContract 双调用点(execute.ts + webhook-console/agent-control.ts)同步。硬验收升级:"只缺 ui_visual_validation → session 终态不得 completed"端到端反例。锚点纪律:符号名+代码特征定位,禁依赖死行号(HEAD 4d86bbc5)。
- **HOTFIX 两案教训(2026-07-09,固化为验收铁律)**:T2.6 的退出治理与 findNewestPng 修复均"实测通过"交付但**零常驻测试**(前者的"10s 退出机判"仅是报告中的一次性脚本,后者的单测场景 600s 陈旧与真实场景 mtime 越窗不等价)——场景漂移后双双复发,由用户实证发现。铁律:**凡修复类交付,验收必须含"钉住用户真实复现序列"的常驻回归测试**(修前 FAIL 修后 PASS 双向证明),"实测过"不算数;报告中的机判脚本必须落进 vitest/CI 常跑集。已补:exit-timing.test.ts(真实 dist 冷道 e2e)与 run.test.ts 真实场景回归。技术修复:flushAndExit 补 250ms hardExit 兜底(退出不依赖 drain 回调);gradle 跑前 prune 本组件 *_actual/*_compare(新鲜度门降为二道防线)。
- **M3 关闭(2026-07-09,Codex 审查通过 round 2)**:北极星达成——m3-t35-demo.sh exit 0(真实 verify-page 5 格全绿 + ui_change 经 loop 全自动至 completed + 回归门⊆基线)。审查修复:classification 消费闭环(environment_gap 阶梯归类不入模型修正回路)、verify-page --version 消歧。Codex 线程曾随进程异常丢失,经 orchestration.md 决断史注入无损续接(经验:决断记录落文档而非只存对话,是跨会话韧性的关键)。
- **M4 裁定(2026-07-09,子计划 round 1 批准)**:T4.1 架构=ui-verify-mcp→uiv-cli(commands.ts"CLI 编排复用层":无 console.log/无进程退出/无 MCP 专属逻辑/只返回结构化结果)→uiv-core;T4.2 证据口径=禁止无证据 fail/uncertain、允许零差异 pass 无证据、pass 带 evidence 也须校验坐标或忽略;T4.4 backlog;T4.5 最小 advisory 版获准;stopOdiffServer CLI+MCP 双层 finally=幂等双保险(有意为之勿删)。
- **M4 关闭 + 全项目收官(2026-07-09,Codex code review round 2 通过)**:round 1 唯一 blocking——check-t43 的 `git checkout --` 复原会吞用户未提交改动 → 修复为写偏前 `git diff --quiet HEAD -- path` 断言(脏则 exit 2 拒跑且不碰文件)+ 原字节 Buffer 复原(d60fbeb);两条建议同落实:attach.ts pass verdict 的 evidence 非数组 → throw 走 l3-attach exit 1 分层、字段缺失仍视为合法空(bb179c9);ci-gate.md 补"demo 全量含 advisory 测试"CI 语义说明防平台默认把 T4.5 被动升级为硬门禁(17c0c0c)。npm test 359/359(基线 358+1)。Codex 确认收官:M0~M4 均审查关闭,两北极星机判达成(Phase 0 盲修 2 轮至 pass / m3-t35-demo.sh exit 0);遗留项全数按 meta.json pending_followups 管理,不构成阻断。**Release Gate 口径不变(§4)**:FIGMA_PAT 真实 REST 闭环验证(交叉标定+fixture 重录+T1.2 复跑)完成前,项目状态为"fixture 驱动实现完成",不得宣称 REST 通道闭环。
- **遗留项1 完成 / Release Gate 解锁(2026-07-09,Codex 口径 C 裁定)**:FIGMA_PAT 到位触发 Release Gate 解锁项(交叉标定+fixture 重录+T1.2 复跑)。①**凭据排障**:PAT 账户 ≠ 标定文件归属账户(uiv-calibration 建于 MCP 授权账户 zhuxi8518@gmail.com,PAT 来自另一账户 → REST 404 Not found;`/v1/me` 403 err body 列出 scope 反证 token 有效),经 zhuxi8518 共享文件给 PAT 账户解锁(非切账户/迁文件,最省事);.gitignore 预配挡 token 入库(bf48082)。②**交叉标定机判 PASS**:真实 REST(page 0:1 仅 frame 1:2,children 1:3~1:6)经产品 `normalizeNodesResponse` 与 canonical 手工 fixture(1:100 系)抹 id/fileKey/version 标签后全树逐字段全等(bbox/cornerRadii/fills/text/padding);CalibSwatch re-base (12,60)==MCP 记录值,C5 口径实证一致;差异仅标签(nodeId/version/fileKey)。③**Codex 裁定口径 C**:1:100 保留为标定合同稳定内部别名,禁迁 nodeId 体系(A/B 账目对齐收益 < 工程扰动,YAGNI+方向锚定②);"重录 fixture"收窄为:真实 REST fixture 留存(`rest-nodes-card.real.json` 字节级)+ 常驻 `cross-calib.test.ts`(4 项,篡改几何必 FAIL 经对抗验证)+ 落账;`rest-nodes-componentset.json` 保持合成 fixture(真实文件无 COMPONENT_SET)。④**REST `/v1/images?scale=2`** 独立最小验证:images 200 + PNG 720×400 PASS(不替换 baseline)。⑤npm test 363 全绿(359→+4)。**Release Gate 升级:项目状态从"fixture 驱动实现完成"→"REST 通道闭环验证通过"**。commit b40de80。方法论固化:承重口径分歧(nodeId 迁移边界)按方向锚定②走决策链交 Codex;主会话亲做凭据敏感+承重侦查(交叉标定判定),机械执行(fixture/测试/images)派 workflow 三 agent(并行 fixture+images,末置对抗验证 agent 防假绿)。
