...
**设计稿漂移哨兵**：`uiv baseline pull --check-version --file hH7NUAlm9DsLRaGScQP0Z1` 比对 mapping.json 钉住的 version 与 `GET /v1/files/:key/meta` 最新 version（Tier 3，轮询便宜），不一致**只告警不阻断**——设计更新走人工触发重录基准的流程，避免设计师改稿静默污染验收信号。
...
| 子命令 | 作用 | 必填参数 | 可选参数 |
|--------|------|----------|----------|
| `pin` | dev_preparation 阶段一条命令固化 parity contract：拉基准、钉 version、写 mapping.json（uiParity 的 source of truth，条目带 source document 作用域 + `states[]`）、触发合同 re-persist 由构建器合并进 RequirementContract。目标若为 COMPONENT_SET 则自动枚举 variant 各落一份分状态基准（CS6） | `--file <fileKey>` `--node <nodeId>` `--test <测试FQN>` `--demo <demo工程目录>` | `--fixture <nodes夹具>`（无 `FIGMA_PAT` 时必给）、`--source <docPath>`、`--state <name>=<variantNodeId>`（可重复）、`--min-score <(0,1]>`、`--matrix <l-shape\|full\|custom:名>` |
| `baseline pull` | 常规模式拉取基准（落 spec.json + baseline.png，写 mapping.json）；加 `--check-version` 切设计稿漂移哨兵模式（告警不阻断，exit 恒 0） | 常规：`--fixture <nodes夹具>` `--file <fileKey>` `--node <nodeId>`；哨兵：`--check-version` `--file <fileKey>` | 哨兵：`--meta-fixture <meta夹具>` |
| `check` | 组件级内循环单轮：渲染 + L1/L2 + report.json；含防震荡 regression 检测；exit = pass ? 0 : 1 | `--preview <PreviewFQN>` `--node <nodeId>` `--demo <demo工程目录>` | `--version <v>`、`--ignore-region <x,y,w,h>`、`--record`（pass=false 拒录，exit 3） |
| `verify-page` | 整页级外循环：设备矩阵 × 内容态矩阵（默认 L 形交叉，见 3.3），聚合 page-report.json；exit = pass ? 0 : 1 | `--test <测试FQN>` `--node <nodeId>` `--demo <demo工程目录>` `--session <id>` | `--version <v>`、`--states <a,b,c>`、`--matrix <l-shape\|full\|custom:名>`、`--out <path>`、`--json` |
| `report` | report.json / page-report.json → JUnit XML（纯转换，exit 恒 0，供 CI 平台展示） | `--junit` `--in <report.json>` | `--out <path>`、`--suite <name>` |
| `l3-attach` | L3 轻量形态 verdict 回填 page-report（无证据判定丢弃，见 2.7） | `--report <page-report.json>` `--verdicts <verdicts.json>` `--pack <l3-input.json>` | — |

示例（仓内真实值，均可直接解析）：

```bash
# pin：固化 parity contract（fixture 通道；设 FIGMA_PAT 走 REST 时可省 --fixture）
uiv pin --file hH7NUAlm9DsLRaGScQP0Z1 --node 39:10844 \
  --test com.magpie.uiv.demo.HashtagPanelScreenshotTest --demo demo-android \
  --fixture packages/uiv-core/fixtures/rest-nodes-yanhao.real.json \
  --source docs/ui-visual-self-verification.md --min-score 0.9 --matrix l-shape

# pin：设计师手画的独立状态 frame 用 --state 显式钉（可重复）
uiv pin --file FKEY --node 9:100 --test com.magpie.uiv.demo.CalibCardScreenshotTest \
  --demo demo-android --fixture packages/uiv-core/fixtures/rest-nodes-componentset.json \
  --state empty=9:102 --state error=9:103

# baseline pull：拉取基准（fixture 模式）
uiv baseline pull --fixture packages/uiv-core/fixtures/rest-nodes-yanhao.real.json \
  --file hH7NUAlm9DsLRaGScQP0Z1 --node 39:10844

# baseline pull --check-version：设计稿漂移哨兵（告警不阻断）
uiv baseline pull --check-version --file hH7NUAlm9DsLRaGScQP0Z1

# check：组件级内循环单轮
uiv check --preview com.magpie.uiv.demo.UploadContentPreview --node 39:10822 --demo demo-android

# verify-page：整页级外循环
uiv verify-page --test com.magpie.uiv.demo.UploadContentPageScreenshotTest --node 39:10822 \
  --demo demo-android --session ci --states typical,longText --json \
  --out .ui-verify/reports/ci/page-report.json

# report：page-report → JUnit XML
uiv report --junit --in .ui-verify/reports/ci/page-report.json --out .ui-verify/reports/ci/junit.xml

# l3-attach：L3 verdict 回填
uiv l3-attach --report .ui-verify/reports/39-10822@2342874355766877359/page-report.json \
  --verdicts .ui-verify/reports/39-10822@2342874355766877359/l3/verdicts.json \
  --pack .ui-verify/reports/39-10822@2342874355766877359/l3/l3-input.json
```

行为约定：所有输出落 cwd 内 `.ui-verify/`，stdout 最后一行打印本次主产物绝对路径（check=report.json、baseline pull=spec.json、pin=mapping.json、verify-page=page-report.json），check/verify-page 的 exit 0/1 即验收门（用法错误 exit 2）。check 命中 daemon 走热路径（UDS），daemon 未启动降级 spawn `./gradlew --no-daemon` 冷路径。
...
uiv check --preview com.magpie.uiv.demo.UploadContentPreview --node 39:10822 --demo demo-android
# MCP
ui_check { previewFqn: "com.magpie.uiv.demo.UploadContentPreview", figmaNodeId: "39:10822" }
```
...
- 轮数上限 5；
- 全过 → 原 check 命令加 `--record` 重跑落 golden（pass=false 时拒录，exit 3），进入下一组件。
...
```bash
uiv verify-page --test com.magpie.uiv.demo.UploadContentPageScreenshotTest --node 39:10822 \
  --demo demo-android --session standalone --states typical,empty,longText
```
...
3. **`/Users/zhuxi/AI/magpie_agent/src/capabilities/loop/application/execute.ts`**
   - 在 integration_test / green_fixup 阶段后期、`validateRequirementContract(session)` 调用点（工作区 L6682 / HEAD L6675）之前，当合同携带 uiParity 指纹时，经 `runSafeCommand()`（`/Users/zhuxi/AI/magpie_agent/src/capabilities/workflows/shared/runtime.ts` L1804，execFileSync 包装、返回 `{passed, output}`）执行：
     ```bash
     uiv verify-page --test com.magpie.uiv.demo.UploadContentPageScreenshotTest --node 39:10822 \
       --demo demo-android --session sess-0042 --json \
       --out .magpie/sessions/sess-0042/ui-visual-validation.json
     ```
     （`--test`/`--node`/`--demo` 取自合同 uiParity 条目、`--session` 取当前 session id，此处为示例值）然后 `session.artifacts.uiVisualValidationPath = 该路径`。
...
- **UI parity 硬门禁（还原度验收，唯一）**：`uiv verify-page` exit code 即门禁，L2 结构断言驱动；`uiv report --junit --in .ui-verify/reports/ci/page-report.json --out .ui-verify/reports/ci/junit.xml` 转 JUnit XML 供 CI 平台展示。这是"像不像设计稿"的唯一裁决者。
...
- **设计稿漂移哨兵**：CI 定期跑 `uiv baseline pull --check-version --file hH7NUAlm9DsLRaGScQP0Z1`，钉住的 version 与 `/meta` 最新 version 不一致**只告警不阻断**，重录基准走人工触发。
...
| 把工具链故障类 inconclusive 喂给模型修代码 | `semantics_export_failed` / `render_harness_error` 是环境问题，让模型"补 tag/改实现"会白烧轮次还可能改坏没错的代码；按 subReason 归类到 environment_gap（编译失败属模型代码写坏，已归 `implementation_gap`，不计入此类） |
