...
import { odiffCompare } from './server.js';
import type { OdiffMode } from './server.js';
...
export async function runL1(baselinePng: string, renderedPng: string, diffOut: string,
                            ignore: IgnoreRegion[], mode?: OdiffMode): Promise<PixelResult> {
  const r = await odiffCompare(baselinePng, renderedPng, diffOut, {
