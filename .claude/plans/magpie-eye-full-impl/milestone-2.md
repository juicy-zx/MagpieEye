...
`equivalence.sh`(bash,`set -euo pipefail`,cd WS;变量 CARD(事实源)、`R=.ui-verify/reports/1-100@T1_0A_V1/report.json`、`T=.calib-tmp/t2.1`(mkdir -p)、`S=.ui-verify/daemon.sock`、`BAK=$T/CARD.orig`;子命令 case):
- `deviate`:`git diff --quiet -- "$CARD"` 否则 exit 3;`cp "$CARD" "$BAK"`(临时备份,先于写偏,供 `diff` 子命令原样恢复,不经 git,避免误踩 staged 变更);`sed -i '' 's/12\.dp to 36\.dp,/20.dp to 44.dp,/' "$CARD"`(subtitle +8,+8→稳定 position 违规);`grep -q '20.dp to 44.dp' "$CARD" && echo DEVIATED`;
...
- `diff`:`node scripts/t2.1/diff-reports.mjs "$T/cold.json" "$T/hot.json"`(继承其 exit);`mv "$BAK" "$CARD"`(备份恢复,非 `git checkout`,不触碰 git 索引/暂存区);echo RESTORED。
...
**前置(Wave 协调)**:依赖 T2.1 的 `selectGradleRunner` 已落地——T2.1 Step 3 已将本文件 check 分支接线为 `const sel = await selectGradleRunner(uiVerifyDir); console.error(\`uiv: gradle lane=${sel.lane} (${sel.reason})\`);`,且 `runCheckL2` 首参已改 `sel.runner`。本步骤在此基础上续接,不再自建/复述 SpawnGradleRunner。

①`packages/uiv-cli/src/index.ts` 导入 RecordRefusedError/runRecord;在 T2.1 接线产出的 `const sel = await selectGradleRunner(uiVerifyDir);` 后加一行 `const runner = sel.runner;`;`runCheckL2` 后、打印 reportPath 前插:
...
describe「降级 1 tie-break:重复文本不错配」单 it(盲区留痕参考 `lcs.ts` 头注释风格):`const s3 = root([fig('3:1','L1','TEXT',10,10,60,20,{characters:'OK'}), fig('3:2','L2','TEXT',10,200,60,20,{characters:'OK'})]); const m2 = matchThreeTier(s3, dump([sem(null,'OK',10,10,60,20), sem(null,'OK',10,200,60,20)]), comparableNodes(s3,[]))`(两个 TEXT 叶子文案相同、均未打 tag,全走降级 1)。断言:`m2.pairs.find((p)=>p.figma.id==='3:1')?.sem.positionDp`→`toEqual({x:10,y:10})`;`m2.pairs.find((p)=>p.figma.id==='3:2')?.sem.positionDp`→`toEqual({x:10,y:200})`;两者 `joinSource`→均 `toBe('text')`。**预期行为**:降级 1 对同分候选按语义 (y,x) 序取靠前者(既有确定性 tie-break,非新增逻辑),fig 侧同序遍历,故渲染顺序与 spec 顺序一致的常规场景(本用例)天然按位置正确配对而非交叉错配;若渲染顺序与 spec 顺序交叉(反直觉场景),该 tie-break 仍可能错配——盲区与 `lcs.ts` 头注释一致,不在本降级层修复,由 tag 强约束路径兜底。
...
(熔断态 `fused` 时上述循环不执行,`violations` 仍为空数组——与既有"熔断不输出 violations"不变式一致,反例见验收⑤;非熔断态下每个 `structural.missing` 条目额外计入一条 high 违规,计入 verdict 的 blockingSeverities 命中,迫使 `pass:false`;`structural.missing` 数组本身不受影响。)score 行改 `const sc = fused ? 0 : score(violations, executed);`;structural 与 subReason 替换为:
...
2. `l2/report.test.ts` 端到端 4 类用例:badge 缺失致 mr=0.75<0.8 触熔断吞 violations——仅该用例内(不动共享 calibSpec)加第 5 叶子使 mr=0.8:spec push `{id:'1:105',name:'CalibFooter',type:'RECTANGLE',absoluteBoundingBox:{x:112,y:260,width:120,height:16}}`,bad dump push `sem('fig:1:105',24,320,240,32)`;断言 untaggedCoverage 0.75→0.8;`violations` 断言因本章新增的 missing→violation 判定多一条(badge 1:104 缺失计入一条 `missing`/high),改 `toEqual(['color','fontSize','missing','position'])`(字典序四条,原三条不变);其余不变。
...
describe「T2.5 验收六类」六个 it 逐条落码(除注明外 r = `runL2(SPEC(), d, {})`):
...
- **⑥missing 硬失败(N=20 缺 1 叶,coverage/matchRate 均达标仍不得 pass)**:`const leaves = Array.from({ length: 20 }, (_, i) => fig('9:' + i, 'L' + i, 'RECTANGLE', i * 20, 0, 16, 16)); const s = root(leaves); const d6 = dump(leaves.slice(1).map((n) => sem('fig:' + n.id, null, n.absoluteBoundingBox!.x, n.absoluteBoundingBox!.y, n.absoluteBoundingBox!.width, n.absoluteBoundingBox!.height))); const r = runL2(s, d6, {});`(20 叶子中 19 个精确 tag 配对,1 个(`9:0`)无对应语义节点)→ `r.structural?.untaggedCoverage` `toBe(0.95)`;`r.structural?.matchRate` `toBe(0.95)`(均高于阈值:cov≥0.9 不触发 tag_coverage_low,mr≥0.8 不熔断,`r.subReason` `toBeNull()`;19 个配对叶子几何完全对齐,无其他违规)→ `r.structural?.violations` `toContainEqual(expect.objectContaining({property:'missing',severity:'high'}))`;`r.pass` `toBe(false)`(Codex M2 审查裁定:missing 必须硬失败,不因 coverage/matchRate 达标而放行)。

验收:`npm test`(158+新增≈12)与 `npm run build` 均 exit 0,无 skip。commit `T2.5: 验收六类端到端用例`。
...
- **通道分流**:文本节点(colorHex≠null)恒走语义通道 `judgePath:'parity'`;非文本**叶子**(colorHex=null 且 children 空)且 spec 首 fill 为 SOLID → 像素通道,violation `judgePath:'parity-pixel-sampled'`、`property:'color'`、high、容差同 `TOL_DELTA_E`、hint 模板不变;fill 非纯色或容器(子像素污染)→ 跳过并记 `structural.diagnostics.pixel`;无 pixelSource → 通道不执行(值不可得)。executed 仅在真正采样比对时 +1。
...
行为红绿由 Step 5 承担:贯通前 D3 必 miss(`--verify-detection` 3/4 exit 31 = 红),贯通后 4/4;本步过门 = build + `npm test` 全绿。**diagnostics 结构统一(承接 T2.5)**:T2.5 已将 `StructuralV1.diagnostics` 定义为对象 `{ containerMissing: Array<{ figmaId: string; name: string }> }` 并只填 containerMissing;本步骤在同一对象上追加 `pixel: PixelDiagnostic[]` 键,不新增并列字段(`diagnostics` 全文统一为对象形态,不得回退成裸数组)。改动:

- `report/v1.ts`:`StructuralV1.diagnostics` 类型扩展为 `{ containerMissing: Array<{ figmaId: string; name: string }>; pixel: PixelDiagnostic[] }`(补 `PixelDiagnostic` type import;只加 `pixel` 键,不改动 `containerMissing`);`checkStructural` 内、T2.5 已加的 `diagnostics.containerMissing` 非数组即 fail 检查旁,追加同风格一条:`diagnostics.pixel` 非数组即 fail。**存量修正**:`report/v1.test.ts` 两处 structural 字面量(T2.5 已补 `diagnostics:{containerMissing:[]}`)追加 `pixel:[]` → `diagnostics:{containerMissing:[],pixel:[]}`,否则被新增校验判 fail。
- `l2/report.ts`:`RunL2Opts` 追加 `pixelSource?: { png: DecodedPng };`(不透传 insetRatio,YAGNI;补相应 type imports);断言循环(T2.5 已包进的 `if (!fused) { … }` 块)前插入下方 pixelCtx 构造与 `const pixelDiagnostics: PixelDiagnostic[] = [];`(局部变量特指本步骤的像素跳过记录,与 structural.diagnostics 字段名区分,避免混淆),循环内 `assertPair(pair)` 改 `assertPair(pair, pixelCtx)` 并追加一行 `for (const d of r.diagnostics) pixelDiagnostics.push(d);`;找到 T2.5 落地的 `diagnostics: { containerMissing: m.containerMissing.map(idName) },`(structural 字面量内)原地追加一键,改为 `diagnostics: { containerMissing: m.containerMissing.map(idName), pixel: pixelDiagnostics },`(**不是**在 `violations,` 后新增一个并列的 `diagnostics,` 键——那会与 T2.5 已有的 diagnostics 键重名冲突):
...
`npm test` 全绿(既有 runCheckL2 用例 8×8 假 PNG 采样越界只落 diagnostics.pixel,断言不变)→ `git commit -m "T2.7 step3: pixelSource 贯通"`。

#### Step 4 — harness D3 回归 CalibSwatch(红→绿→commit,D-04 ③)

红:改 `scripts/phase0-lib.test.mjs` —— ① `expect(out).toContain('Color(0xFF99B3E6)'); // D3` 改为两行 `expect(out).toContain('Color(0xFFFF6600)'); // D3` 与 `expect(out).toContain('Color(0xFFCCE0FF)'); // subtitle 不再写偏`;② 检出门 fixture 中 `{ property: 'color', testTag: 'fig:1:102' }`(assertSeededDetection describe 块内)改 `'fig:1:103'`;**不改**同文件 `expect(out).toContain('12.dp to 36.dp,  // fig:1:102 CalibSubtitle')`——该断言是 CHILD_POSITIONS 表 subtitle 项的位置契约,subtitle 节点 id 本身未变、与 D3 目标属性(color)无关,继续留在 1:102(其余三条 CHILD_POSITIONS 项断言同理不动)。
另改 `scripts/phase0-acceptance.smoke.test.mjs` 的 `failReport.structural.violations[2]`(现 `{ judgePath: 'parity', testTag: 'fig:1:102', figmaName: 'CalibSubtitle', property: 'color', ... }`):`testTag` 改 `'fig:1:103'`、`figmaName` 改 `'CalibSwatch'`。此 mock 直接喂给真实 `assertSeededDetection`(未 stub),D3 目标改后若不同步,该文件 `--verify-detection` 冒烟用例会从 hit 4/4 退化为 3/4,拖累 `npm test`。
...
收尾机判(**收窄口径,只查 D3 定义点,非整文件/repo 全量 1:102 清零**):`grep -n '1:102' scripts/phase0-lib.mjs scripts/phase0-config.json` 零命中(D3 的 deviation 描述与 `assertSeededDetection` 目标均已迁至 1:103);不对 `phase0-lib.test.mjs`/`phase0-acceptance.smoke.test.mjs` 做整文件 1:102 清零断言——这两个测试文件除已随 D3 迁移的 fixture 外,还留有与 D3 无关、作为合同断言存在的 `fig:1:102`(CalibSubtitle 位置契约),不应被清除。
`npx vitest run scripts/phase0-lib.test.mjs scripts/phase0-acceptance.smoke.test.mjs` 绿 → `git commit -m "T2.7 step4: D3 回归 CalibSwatch"`。
...
node -e "const r=require('./.ui-verify/reports/1-100@T1_0A_V1/report.json');const cd=r.structural?.diagnostics?.pixel?.some(d=>d.code==='pixel_sample_skipped_container'&&d.testTag==='fig:1:100');if(r.pass!==true||!cd)throw new Error('正确态误报或容器 diagnostics 缺失');console.log('零假阳性 pass:true + 容器跳过已记录')"
```

预期:`hit:4/4`;正确态 swatch/badge ΔE=0 不误报,根容器落 diagnostics.pixel;`git status --porcelain` 为空。
...
| 3 | Step 5 两段 `node -e` | D3 走像素通道;正确态 `pass:true` + 容器跳过落 diagnostics.pixel |
